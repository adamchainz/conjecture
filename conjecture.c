...
static conjecture_buffer *conjecture_buffer_new(size_t capacity){
  conjecture_buffer *buffer = (conjecture_buffer*)malloc(sizeof(conjecture_buffer));
...
void conjecture_assume(conjecture_context *context, bool condition){
  if(!condition) conjecture_reject(context);
}
...
void conjecture_draw_bytes(conjecture_context *context, size_t n, unsigned char *destination){
...
uint64_t conjecture_draw_uint64_under(conjecture_context *context, uint64_t max_value){
  if(max_value == 0){
    return 0;
  }
...
int64_t conjecture_draw_int64_between(conjecture_context *context, int64_t lower, int64_t upper){
  assert(lower <= upper);
  if(lower == upper) return lower;
  if((lower == MIN_INT64) && (upper == MAX_INT64)) return conjecture_draw_int64(context);
...
static bool is_failing_test_case(
  conjecture_comms *comms,
  conjecture_buffer *buffer,
  conjecture_test_case test_case,
  void *data
){
...
static void buffer_copy(conjecture_buffer *destination, conjecture_buffer *source){
  assert(destination->capacity == source->capacity);
  destination->fill = source->fill;
  memmove(destination->data, source->data, source->fill);
}

static void buffer_delete_range(conjecture_buffer *buffer, size_t start, size_t end){
  assert(end >= start);
  if(end >= buffer->fill){
    buffer->fill = start;
    return;
...
static bool shrink_buffer(conjecture_buffer *destination, conjecture_buffer *source, uint64_t stage){
...
void conjecture_run_test(
  ...
  while((good_examples < runner->max_examples) && (total_examples < 10 * runner->max_examples)){
    good_examples++;
    total_examples++;
  ...
  if(found_failure){
    ...
    printf("\n");
    conjecture_buffer *secondary = conjecture_buffer_new(runner->max_buffer_size);
    bool changed = true;
    int shrinks = 0;
    int extra_tries = 0;
    while(changed){
...
void conjecture_variable_draw_start(
  conjecture_variable_draw *variable, conjecture_context *context, size_t object_size
){
